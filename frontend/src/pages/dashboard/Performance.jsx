...
const Performance = () => {
  ...
  return (
    <div className="space-y-6 text-slate-800">
      ...
      {activeTab === 'Level Performance' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col items-center justify-between">
            <h3 className="text-base font-bold text-slate-800 w-full text-left mb-4">Target Achievement by Level</h3>
            <div className="h-56 w-full flex items-center justify-center relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={achievementByLevelData} cx="50%" cy="50%" innerRadius={70} outerRadius={90} paddingAngle={4} dataKey="value">
                    {achievementByLevelData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              ...
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-3xl font-black text-slate-850">{totalAgents > 0 ? '100%' : '0%'}</span>
                <span className="text-xs text-slate-400 font-bold uppercase mt-0.5">Overall</span>
              </div>
            </div>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-4">
            <h3 className="text-base font-bold text-slate-800">Target Breakdown by Level</h3>
              ...
              {achievementByLevelData.map((d, idx) => (
                <div key={idx} className="flex justify-between items-center border-b border-slate-100 pb-3 last:border-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-extrabold text-slate-700">{d.name}</span>
                  </div>
      ...
      {activeTab === 'Target Achievement' && (
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <h3 className="text-base font-bold text-slate-855 mb-4">Overall Performance Trend</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  ...
                  <YAxis fontSize={9} stroke="#64748b" />
                  <Tooltip />
                  <Line type="monotone" dataKey="target" stroke="#3b82f6" strokeWidth={2.5} name="Target Achievement (%)" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {activeTab === 'Revenue Performance' && (
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <h3 className="text-base font-bold text-slate-855 mb-4">Revenue Trend</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  ...
                  <YAxis fontSize={9} stroke="#64748b" />
                  <Tooltip />
                  <Line type="monotone" dataKey="revenue" stroke="#f59e0b" strokeWidth={2.5} name="Revenue Achievement (%)" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      {activeTab === 'Activity Performance' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            <h3 className="text-base font-bold text-slate-855 mb-4">Activity Completion Trend</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  ...
                  <YAxis fontSize={9} stroke="#64748b" />
                  <Tooltip />
                  <Line type="monotone" dataKey="activity" stroke="#10b981" strokeWidth={2.5} name="Activity Completion (%)" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-4">
            <h3 className="text-base font-bold text-slate-800">Activity Metrics</h3>
            <div className="space-y-4">
              {keyMetrics.map((m, idx) => (
                <div key={idx} className="space-y-1.5 text-xs">
                  <div className="flex items-center justify-between font-bold">
                    <span className="text-slate-555">{m.label}</span>
      ...
      {activeTab === 'Top Performers' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            <h3 className="text-base font-bold text-slate-800 mb-4">Top Performers (Agents)</h3>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex flex-col justify-between">
            <h3 className="text-base font-bold text-slate-855 mb-4">Bottom Performers (Agents)</h3>
            <div className="space-y-4 text-xs font-bold text-slate-655 font-semibold">
              {bottomPerformers.map((a, idx) => (
                <div key={idx} className="flex justify-between items-center border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                  <div>
                    <span className="text-slate-855 font-extrabold block">{a.name}</span>
